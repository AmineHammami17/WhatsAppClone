...
export default function Authentification({ navigation }) {
  ...
  const handleSignIn = async () => {
    ...
    try {
      const auth = firebase.auth(); // Access the auth instance correctly
      await auth.signInWithEmailAndPassword(email, password);
      const currentid = auth.currentUser.uid; // Get the current user ID
      navigation.replace("Acceuil", { currentid: currentid }); // Navigate to the next screen with the user ID
